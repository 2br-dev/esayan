...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if ($controller == 'getProduct'){
	    $response = array();
	    $id = $_POST['id'] ? $_POST['id']: '';
	    $category = $_POST['category'];
	    $file = new \Files();
	    if($id){
            $product = Q("SELECT * FROM `#_mdd_".$category."` WHERE `visible` = 1 AND `id` = ?i", [$id])->row();
            $product['image_file'] = $file->getFilesByGroup($product['image'], ['original'], ['file'], true);
            if(!empty($product)){
                $ico_id = explode(',', $product['ingredients']);
                foreach($ico_id as $key => $value){
                    if($product['color_ico'] == 'light'){
                        $product['ico'][$key] = Q('SELECT `title`, `icon_light` as `icon` FROM `#_mdd_ingredients` WHERE `id` = ?i', [$value])->row();
                    }
                    else {
                        $product['ico'][$key] = Q('SELECT `title`, `icon_dark` as `icon` FROM `#_mdd_ingredients` WHERE `id` = ?i', [$value])->row();
                    }
                }
                foreach ($product['ico'] as $key1 => $value1){
                    $product['ico'][$key1]['ico_file'] = $file->getFilesByGroup($value1['icon'], ['original'], ['file'], true);
                }
            }
            $response = $product;
        }
